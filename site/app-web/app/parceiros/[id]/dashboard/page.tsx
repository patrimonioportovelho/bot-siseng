...
export default async function DashboardCorretorPage({
  ...
  const [
    ...
    // Produção mês a mês: repasses já pagos a este parceiro, pela Data de
    // pagamento — alimenta o gráfico "Produção x Queda" abaixo.
    prisma.movimentacoes.findMany({
      where: { tipo: "Despesa", pago: true, parceiro_id: id, data_pagamento: { gte: inicio, lt: fimExclusivo } },
    ...
    prisma.movimentacoes.aggregate({
      _sum: { valor: true },
      _count: true,
      where: { tipo: "Despesa", pago: false, parceiro_id: id, vencimento: { gte: inicio, lt: fimExclusivo } }
    })
  ]);
    ...
    <div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-2 gap-3 mb-5">
        <div className="bg-gray-50 border border-gray-100 rounded-xl p-3">
          ...
          <div className="text-lg font-bold mt-1 text-accent">{formatMoeda(despesasPagasTotal._sum.valor ?? 0)}</div>
          <div className="text-[11px] text-gray-400">{despesasPagasTotal._count} repasse(s) pago(s)</div>
        </div>
        <div className="bg-gray-50 border border-gray-100 rounded-xl p-3">
          <div className="text-xs text-gray-500">A receber no período</div>
          <div className="text-lg font-bold mt-1 text-gray-900">
            {formatMoeda(despesasPendentesTotal._sum.valor ?? 0)}
          </div>
          <div className="text-[11px] text-gray-400">{despesasPendentesTotal._count} pendente(s)</div>
        </div>
      </div>
